...
#define ILI9488m_swap(a, b) { typeof(a) t = a; a = b; b = t; }
#endif
...
void Teensy_Parallel_GFX::fillTriangle ( int16_t x0, int16_t y0,
  ...
  if (y0 > y1) {
    ILI9488m_swap(y0, y1); ILI9488m_swap(x0, x1);
  }
  if (y1 > y2) {
    ILI9488m_swap(y2, y1); ILI9488m_swap(x2, x1);
  }
  if (y0 > y1) {
    ILI9488m_swap(y0, y1); ILI9488m_swap(x0, x1);
  }
    ...
    if (textcolor == textbgcolor) {
...

		{
			...
			while (y_top_fill--) {
              ...
              drawPixel(xx, y, (gfxFontLastCharPosFG(xx,y)? _gfx_last_char_textcolor : (xx < x_offset_cursor)? _gfx_last_char_textbgcolor : textbgcolor));
            }
					}					
				}
				y++;
			...
			for(yy=0; (yy<h) && (y < _displayclipy2); yy++) {
                ...
                for (uint8_t xts = 0; xts < textsize_x; xts++) {
                  if ( (x >= _displayclipx1) && (x < _displayclipx2)) {
                    if (bits & 0x80) {
                      ...
                      temp_color = (gfxFontLastCharPosFG(x,y)? _gfx_last_char_textcolor : (x < x_offset_cursor)? _gfx_last_char_textbgcolor : textbgcolor);
                      drawPixel(x, y, temp_color);
                    }
                  }
              ...
              while (x < x_end) {
                if (x >= _displayclipx1) {
                  //write16BitColor(gfxFontLastCharPosFG(x,y)? _gfx_last_char_textcolor : (x < x_offset_cursor)? _gfx_last_char_textbgcolor : textbgcolor);
                  drawPixel(x, y, gfxFontLastCharPosFG(x,y)? _gfx_last_char_textcolor : (x < x_offset_cursor)? _gfx_last_char_textbgcolor : textbgcolor);
                }
                x++;
              }
			    	}
